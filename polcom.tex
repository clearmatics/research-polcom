...
Reduction $\bdv^{\oracles^{\PCOM}}$: On input $\inp$ from the adversary $\adv$
compute the simulated proof as follows, for each Round $i$:
\begin{itemize}
\item Parse the protocol building challenges $\vec{\beta_{i - 1}}$.
\item Ask the simulator for $k_i$ simulated commitments
  $c_{i, 1}, \ldots, c_{i, k}$ that correspond to commitments to
  $k_i$  polynomials $\p{p}_{i, j}$ sent by the prover in the
  round. 
...
\item If the verifier asks to reveal evaluation of a determined polynomial
  $\p{p_{i', j}} (\vec{x_{i', j, k}}, \cdot)$ compute $r_{i', j, k}$ according to the
  previously computed values (cf.~\cref{lem:rand_imp_det}) and the partial
  transcript; then the output it.
...
\item Compute the protocol building and evaluation challenges $\vec{\beta_{i}}$,
  $\vec{x_{i', j, k}}$ for the next round by querying the random oracle on the
  partial transcript.
\end{itemize}

On random oracle query $x$ from the adversary, pass the query to the oracle and
return the answer.
