...
\begin{proof}
  ...
  If $\pf_{j_i} = \pf_{j_{i'}}$, that is some polynomial is the last polynomial
  for more than one verification equation, then pick the last polynomial
  from $\p{F}_{i} \setminus \smallset{\pf_{j_i}}$ and add it to $L$. Continue
  till there is no  
  The simulator proceeds as
  follows: If $\pf_i \not\in L$ then $\simulator$ picks a random polynomial
  $\pf'_i(X)$ ans sends it as $\pf_i$. Alternatively, if $\pf_{j_i} \in L$, then
...
polynomial commitment $\PCOM$. We denote its non-interactie variant compiled using Fiat-Shamir by $\PHP^{\PCOM}_{\mathsf{fs}}$. E.g., we denote the above rolled out non-interactive version of the
...
We describe $\oraclesps$ in terms of calls to $\oraclespcom$.
      \begin{itemize}
...
we build an adversary $\bdv^{\oraclespcom}(\srs_{\PCOM}, \rbdv)$ for the a SE-PCOM.
