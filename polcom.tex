...
\subsubsection{The idealised proof system}
  ...
  \item Let $\p{p}_{i', j'} (\vec{X}, \vec{B})$ be a polynomial that is evaluated at
    ...
    randomisers $B_{i_1}, \ldots, B_{i_k'} \in \smallset{B_1, \ldots, B_l}$.
...
\end{definition}
    ...
    \begin{lemma}
      ...
      Let $\p{p} (\vec{X}, \vec{B})$ be a randomised polynomial that is to be
      evaluated at $k$ vectors $\vec{x_1}, \ldots, \vec{x_k}$ each of length
      $l'$. Then for every $\vec{x_i}$ and randomness $\vec{b}$, there is a PPT
      algorithm $\adv$ that outputs a rational function $\p{p}'(\vec{X}, \vec{B})$
      such that $\p{p}(\vec{x}, \vec{b}) = \p{p}'(\vec{x}, \vec{b'})$.
    \end{lemma}
    \begin{proof}
      Let $\p{p} (\vec{x_i}, \vec{b}) = r_i$. Let $\adv$ be an algorithm that picks
      coefficients of $\p{p}'' (\vec{X}, \vec{B})$ randomly and defines
      $\p{p}' (\vec{X}, \vec{B}) = \p{p}'' (\vec{X}, \vec{B}) \cdot \prod_{j = 1}^{l'} (X_1 \cdot \ldots
    ...
    i$. Eventually $\p{p}' (\vec{x_i}, \vec{b}) = r_i$. \qed
  \end{proof}
  ...
  Let $\adv$ be an algorithm from \cref{lem:matching_R_polys} which for each
  randomised polynomial
  $\p{p}_{i, j} (\vec{X}, \vec{B}) = \p{h}_{i, j}\left(\vec{X}, \vec{\beta},
    (\p{p}_{i, j} (\vec{X}, \vec{B}))_{(i, j) \leq (i', j')} \right)$, evaluation
  challenges $\vec{x_{i, j, k}}$ and randomness $\vec{b}$ produces a polynomial
  $\p{p}'_{i, j} (\vec{X}, \vec{B})$ and randomness $\vec{b'}$ such that for
  $\vec{x} \in \smallset{\vec{x_{i, j, 1}}, \ldots, \vec{x_{i, j, K_{i, j}}}}$ holds
  $\p{p} (\vec{x}, \vec{b}) = \p{p}' (\vec{x}, \vec{b'})$.  Then for all
  $\p{p}_{i, j} \in D$ hold
  \[
  ...
  That is, if $\adv$ produces polynomials $\p{p}'_{i, j}$, for $(i, j) \in R$, as in
