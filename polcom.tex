...
\begin{lemma}
  Let $\PS = (\PS.\prover, \PS.\verifier)$ be a PIOP with soundness error
  $\epss$ where the prover sends up to $\noofp$ polynomials. Let $\PCOM$ be a
  knowledge polynomial commitment scheme with extraction error $\epsk$. Let
  $\PSc = (\PSc.\prover, \PSc.\verifier)$ be a proof system such that
  \begin{description}
  \item[$\PSc.\prover$] acts as $\PS.\prover$, except
    \begin{itemize}
    \item when $\PS.\prover$ sends a polynomial $f$ to the polynomial oracle,
  ...
  \item[$\PSc.\verifier$] acts as $\PS.\verifier$, except when $\PS.\verifier$
    asks oracle $\oracleo(f)$ for an evaluation of $f$ at $z$, $\PSc.\verifier$
    sends $z$ to $\PSc.\prover$ and gets $f(z)$ in return.
  \end{description}
  Then $\PSc$ is sound with soundness error at most $\epss + \noofp \cdot \epsk$.
\end{lemma}
\begin{proof}
  Let $\adv$ be an adversary that breaks soundness of $\PSc$ with probability
  greater than $\epss + \noofp \cdot \epsk$. We build $\bdv$ that breaks
  soundness of $\PS$ with probability greater than $\epss$.

  $\bdv$ starts by picking an SRS $\srs$ for the polynomial commitment scheme
  $\kgen$ and provides $\srs$ to $\adv$. Each time $\adv$ sends a commitment
  $c$, then $\bdv$ uses the extractor $\ext$ to extract polynomial $f$ from $c$
  and sends $f$ to the newly set oracle $\oracleo(f)$.  \michals{5.5}{how much
    we can generalize here? I.e.~could we have a single proof that would cover
  ...
  $\adv$ finalizes its proof. \michals{5.5}{why the $\adv$'s proof breaks
    soundness of $\PS$?}
\end{proof}
