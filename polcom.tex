...
\begin{definition}[Rational SE PCOM]
 ...
 \begin{definition}[Sufficiently simulatable PCOM]
   ...
   $\PS = (\kgen, \prover, \verifier, \simulator)$ a zero-knowledge proof system
...
\section{From PIOP to proof system}
\begin{figure}
  \centering
    \begin{description}
    \item[Input]
      \begin{itemize}
      \item PIOP $\PS = (\prover, \verifier)$
    ...
    \item[Compiler]
      \begin{itemize}
        \item $\PSc.\prover$ acts as $\PS.\prover$, except
          ...
          \end{itemize}
        \item $\PSc.\verifier$ acts as $\PS.\verifier$, except when
          $\PS.\verifier$ asks oracle $\oracleo_f$ for an evaluation of $f$ at
          $z$, $\PSc.\verifier$ sends $z$ to $\PSc.\prover$ and expects $f(z)$
          in return.
        \end{itemize}
    \end{description}  
...
\begin{theorem}
  Let $\PS = (\PS.\prover, \PS.\verifier)$ be a PIOP with knowledge soundness
  error $\epsks$ where the prover sends up to $\noofp$ polynomials. Let $\PCOM$
  be a sufficiently simulatable polynomial commitment scheme for $\PS$ with
  extraction error $\epsext$. Let $\PSc = (\PSc.\prover, \PSc.\verifier)$ be a
  ...
  \item[$\PSc.\verifier$] acts as $\PS.\verifier$, except when $\PS.\verifier$
    ...
    sends $z$ to $\PSc.\prover$ and expects $f(z)$ in return.
  \end{description}
  Then $\PSc$ is simulation extractable with extraction error at most \hl{...}.
\end{theorem}
\begin{proof}
