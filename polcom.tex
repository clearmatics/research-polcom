...
zeroes; and $b_{i, j, k} (\vec{x_{i, j, k}}) = r_{i, j, k}$ and
  ...
  \begin{lemma}
    ...
    $\p{p}_{i, j} (\vec{x}, \vec{b}) = \p{p}'_{i, j} (\vec{x}, \vec{b'})$ for all $(i, j)$.
  \end{lemma}
  \begin{proof}
  ...
  $\p{p}'_{i, j} (\vec{x_{i, j, k}}, \vec{b'}) = \p{p}_{i, j} (\vec{x_{i, j,
      k}})$. Obviously, $R \subset E$. Note that there is a minimal index
    ...
    \begin{split}
      ...
      \p{h}_{i^*, j^*}\left((\vec{x_{i^*, j^*, k}})_{k \leq k^*},
        ...
        \vec{b}))_{(i, j) < (i^*, j^*)} \right) = \\ \p{h}_{i^*, j^*}\left((\vec{x_{i^*, j^*,
            k}})_{k \leq k^*}, (\vec{\beta_i})_{i \leq i^* - 1}, (\p{p}'_{i, j} (\vec{x_{i, j, k}},
        \vec{b'}))_{(i, j) < (i^*, j^*)} \right) = \p{p}'_{i^*, j^*} (\vec{x_{i^*, j^*, k^*}},
      \vec{b'})
    \end{split}
  \end{equation}
  \qed
