...
\item[$(\msg{commit}, rand, d)$:] picks a random polynomial $f$ of degree $d$ and
  ...
  \item[$(\msg{open}, c, z, y)$:] returns an opening $o$ for commitment $c$
    ...
    holds $f(z) = y$.
  \end{description}
  We say that $\PCOM$ is \emph{$(3, \eps)$-aux-simulation extractable} if for
...
%     \p{t_{hi, 1}}(X) - \p{t_1}(\chz)\\
%     & v(\p{t_{lo, 2}}(X) + \chz^\noofc \p{t_{mid, 2}}(X) + \chz^{2 \noofc}
%     \p{t_{hi, 2}}(X) - \p{t_2}(\chz))\\
%     & v^2(\p{t_{lo, 3}}(X) + \chz^\noofc \p{t_{mid, 3}}(X) + \chz^{2 \noofc}
%     \p{t_{hi, 3}}(X) - \p{t_3}(\chz))\\
% 		& + v^3(\p{r}(X) - \p{r}(\chz)) \\
% 		& + v^4 (\p{a}(X) - \p{a}(\chz))\\
% 		& + v^5 (\p{b}(X) - \p{b}(\chz))\\
% 		& + v^6 (\p{c}(X) - \p{c}(\chz))\\
% 		& + v^7 (\p{S_{\sigma 1}}(X) - \p{S_{\sigma 1}}(\chz))\\
% 		& + v^8 (\p{S_{\sigma 2}}(X) - \p{S_{\sigma 2}}(\chz))
% 	\end{aligned}
% 	\right)\\
...
\begin{enumerate}
  ...
  \item In Round 3, it computes polynomials $\p{t_1}(X), \p{t_2}(X), \p{t_3}(X)$
    honestly, however uses trapdoor $\chi$ to compute commitments
...
\begin{enumerate}
  ...
  \item In Round 3, it computes polynomials $\p{t_1}(X), \p{t_2}(X), \p{t_3}(X)$
   such that 
 \end{enumerate}
 \michals{11.08}{Should we write down Plonk such that we show where we use
   which PCOM protocols?}

 \section{Unoptimized $\plonk$ with arbitrary polynomial commitment $\PCOM$}
 \michals{11.08}{write down the protocol in more abstract way}
\subsection{$\plonk$ protocol rolled out}
% \label{sec:plonk_explained}
% \newcommand{\vql}{\vec{q_{L}}}
% \newcommand{\vqr}{\vec{q_{R}}}
...
\begin{enumerate}
 ...
 polynomial commitment scheme.
  ...
  \begin{proof}
    ...
    \item On $\adv$'s challenge instance $\inp$ and proof $\zkproof$.
      ...
      \item Parse $\zkproof$ to obtain
        ...
        \end{itemize} 
      \item Compute
        $\aux = (\cp{a}, \cp{b}, \cp{c}, \beta, \gamma, \p{z}, \alpha, \cp{t_1},
        \cp{t_2}, \cp{t_3})$.
      \item Output
        \begin{itemize}
  ...
  Hence, the extractor $\ext_\adv$ proceeds as follows, it gets as input $\srs,
  \srss$, the list $\qadv$ is as constructed above by $\bdv$, randomness
  $\radv$.
