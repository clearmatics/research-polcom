...
\section{DRAFT-Notes on transformation from PHP to Interactive Arguments}
...
\item[Round 1] Sample $b_1, \ldots, b_9 \sample \FF_p$; compute
  $\p{a}(X), \p{b}(X), \p{c}(X)$ as
	...
	\frac{1}{\p{Z_H}(X)}\\
	\p{t_2}(X) & = ((\p{a}(X) + \beta X + \gamma) (\p{b}(X) + \beta k_1 X + \gamma)(\p{c}(X) 
	+ \beta k_2 X + \gamma)\p{z}(X)) \frac{1}{\p{Z_H}(X)} \\
	...
	\end{align*}
  Output $\gone{\p{t_{1}}(\chi), \p{t_{2}}(\chi), \p{t_{3}}(\chi)}$.
	...
	\begin{aligned}
		& \p{t_{1}}(X) - \p{t_1}(\chz) \\
    & + v(\p{t_{2}}(X) - \p{t_2}(\chz)) \\
    & + v^2(\p{t_{3}}(X) - \p{t_3}(\chz)) \\
		& + v^3(\p{r}(X) - \p{r}(\chz)) \\
		& + v^4 (\p{a}(X) - \p{a}(\chz))\\
		& + v^5 (\p{b}(X) - \p{b}(\chz))\\
		& + v^6 (\p{c}(X) - \p{c}(\chz))\\
