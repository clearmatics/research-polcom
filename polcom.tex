...
challenges picked by the verifier.
   ...
   The following lemma holds.
    ...
    \begin{lemma}
      ...
      $\p{p}_{i, j} (\vec{X}, \vec{B})$ at some evaluation point $\vec{x_{i, j,
          k}}$. Then there is a a PPT algorithm $\adv$ that for any
      $\vec{x'_{i, j, k}}$ outputs a rational function
...
\begin{theorem}
  Let $\PS$ be a proof system output by the compiler $C^{\PCOM}$ \michals{23.12}{Define the
    compiler} such that
  \begin{itemize}
...
\item On proof $\zkproof$ from $\adv$, abort if it is unacceptable.  Else, for all
  openings $o_{i, j, k}$ the proof contains, find corresponding commitments
  $c_{i, j}$ and random oracle queries which answer contain evaluation challenges
  $\vec{x_{i, j, k}}$ for that commitment. Denote by
  $(\vec{c}, \vec{x}, \vec{y}, \vec{o})$ be such that commitments $\vec{c}$ are
  commitments contained in $\zkproof$, $\vec{x}$ their evaluation challenges,
  $\vec{y}$ the evaluations themselves, and $\vec{o}$ commitments' openings which has
  not been output by the $\oracles^{\PCOM}$. $\bdv$ outputs
  $(\vec{c}, \vec{x}, \vec{y}, \vec{o})$.  
\end{enumerate}

\michals{23.09}{need the following property -- the unique response property needs to
  kick in before any of the evaluation challenges is sent}

We note that since in $\PS$ the verifier sends evaluation challenges no sooner than
in Round $\tilde{i}$ and the proof system has $\tilde{i'}$ unique response property,
$\tilde{i'} \leq \tilde{i}$, then with overwhelming probability evaluation challenges
in $\zkproof$ are not included in any simulated proof $\bdv$ produced.

Since the verifier accepts $(\vec{c}, \vec{x}, \vec{y}, \vec{o})$ there exists an
extractor $\extb (\srs_{\PCOM}, \Qsimpc, \Qchal, \Qop, \rb)$ that outputs
$\vec{\p{f}}$ such that $\vec{c} = \com (\vec{f})$.
