...
\begin{theorem}[From WEPIOP to ZK WEPIOP]
  Let $\PIOP = (\prover, \verifier)$ be a WEPIOP and $E$ its encoding set and
  ...
  \begin{itemize}
    \item[$\prover'$] acts as $\prover$ except for $f_i(X) \in E$. Let $k_i$ be a
      ...
      $d_i = \deg(f_i)$, then $\prover'$ 
      computes
      \[
        g_i(X) = f_i(X) + X^{d_i + 1} b_1 + \ldots
        X^{d_i + k_i + 1} b_{k_i + 1}
      \]
      for random $b_1, \ldots, b_{k + 1}$ and sets oracle $\oracleo_{g_i}$.
    \item[$\verifier'$] \michals{24.06}{Do we need to change the verifier?}
  \end{itemize}
\end{theorem}
\begin{proof}
