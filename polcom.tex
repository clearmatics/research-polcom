...
\subsubsection{The idealised proof system}
  ...
  The verifier sends challenges, which we divide into two groups: \emph{protocol
    building challenges} $\beta_{i, j}$ and / or \emph{evaluation challenges}
  $\chz_{i, j}$, where index $(i, j)$ should be interpreted as $j$-th challenge in
  Round $i$. Vector $\vec{X}$ denotes a random variable (or a vector of random
  variables) corresponding to evaluation challenges. That is, for each evaluation
  challenge $\chz_{i, j}$ the protocol specifies which $X \in \vec{X}$ are realised
  by $\chz_{i, j}$. Importantly, \hl{...} \michals{29.11}{Don't recall now what was
    so important here}
  ...
  $\p{p_{i', j'}} (\vec{X}, \vec{B})$ has non-zero coefficient next to $B_i$ while for
  all $\p{p}_{i, j}$ for $(i, j) < (i', j')$, coefficient next to $B_i$ is zero.  Polynomials
  ...
  \item[Prover]
    ...
    \item On evaluation challenge $\chz_{i, j}$ the prover checks if in the already
      set oracles there are polynomials which are evaluation ready. In that case it
      allows the verifier to learn the evaluations. \michals{25.11}{Here the prover
        ``allows'' the verifier to learn some oracle evaluations, maybe it could be
  ...
  \item[Verifier]
    ...
    \item[$i \leq \mu$:] on partial transcript $\zkproof [i - 1]$, prover's messages
      ...
      $\smallset{\oracleo_{\p{p_{i, j}}}}_{i = 1}^{k_i}$ sends in Round $i$ 
      \begin{itemize}
      \item a protocol building challenge $\smallset{\beta_{i, j}}_j$, and / or
      \item a evaluation challenge $\smallset{\chz_{i, j}}_j$.
      \end{itemize}
...
\begin{definition}[$\vec{Z}$-Zero knowledge]
  ...
  \michals{24.11}{Need to formalize that the simulator picks some of the polynomials
    randomly and some such that verification equation holds.} Let
  $\vec{Z} = \smallset{z_{i, j}}_{i = 1, j = 1}^{\mu, k_i}$. Let the protocol be a
  proof system $\PS$ as described in \cref{def:ps} such that
  \begin{itemize}
  \item $\smallset{\p{p}_{i, j} (\vec{X}, \vec{b})}$ be independent;
  ...
  \item On verifier's evaluation query to a polynomial oracle:
    \begin{itemize}
    ...
    \item Else, that is the queried oracle is for a polynomial in $D$, compute
      $p_{i, j} = \p{p}_{i, j} (\vec{x}, \vec{b})$ and pick polynomial $\p{p}'_{i, j}
      (\vec{X}, \vec{B})$ such that $\p{p}'_{i, j} (\vec{x}, \vec{b}) = p_{i, j}$ and
      respond with $p_{i, j}$. 
    \end{itemize}
  \end{enumerate}
...
\begin{theorem}
  ...
  in \hl{PHP zero knowledge reference}.
\end{theorem}
...
\changedm{ Write $C_{2i} - \sum_{j = 1}^{k_i} \frac{C_{3ij}}{x_{ij} - x} = 0$ as
  $C_{2 i} \cdot (x_{i 1} - x) \cdot \ldots \cdot (x_{i k_i} - x) - \sum_{j =
    1}^{k_i} C_{3 i j} \cdot (x_{i 1} - x) \cdot \ldots \cdot (x_{i j - 1} - x) \cdot
  (x_{i j + 1} - x) \cdot \ldots \cdot (x_{i k_i} - x)$. Assuming 
  $x_{ij} \neq x_{i'j'}$ for $(i, j) \neq (i', j')$ we have that
