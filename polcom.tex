...
\subsubsection{The idealised proof system}
  ...
  $j$-th challenge in Round $i$ and \emph{evaluation challenges} $\chz_{i, j, k}$,
  ...
  challenges. That is, for each evaluation challenge $\chz_{i, j, k}$ the protocol
  ...
  Assume that
  $\p{p}'_{i, j} (\vec{X}, \vec{B}) = \p{h}'_{i, j}\left(\vec{X}, \vec{B},
    \vec{\beta}, (\p{p}_{i, j} (\vec{X}, \vec{B}))_{(i, j) \leq (i', j')} \right) $
  are polynomials such that
  $\p{p}'_{i, j} (\vec{x}, \vec{b}) = \p{p} (\vec{x}, \vec{b})$ for $\p{p} \in R$. We
  then show that for all $\p{p}_{i, j} \in D$ holds
  \[
