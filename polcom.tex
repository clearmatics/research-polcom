...
% \begin{equation}
%   \left\{ (\p{p}_{i, j} (\vec{x_{i, j, 1}}, \vec{b}))_{(i, j)} \times \ldots \times \right\}_{\vec{x_{i, j,
%         k}}} \approx \dist{P}_{i, j, k}.\label{eq:distribution_P}
% \end{equation}
%
%\michals{3.1}{I think we needed more than just $\dist{P}_{i, j, k}$.}
%
...
\begin{lemma}
  Let $\p{p} (\vec{X}, \vec{B})$ be a randomized polynomial evaluated at $K$ points $\vec{X_1},
  \ldots, \vec{X_k}$. Write $\p{p} (\vec{X}, \vec{B}) = \p{p'} (\vec{X}) + \p{b}
  (\vec{X})$ such that $\p{p'}$ is a polynomial compound of all monomials in $\p{p}$ that
  are independent of $\vec{B}$. \michals{3.1}{I am not sure how to write well that
    sometimes we take $\vec{B}$ as a variable and sometimes as a
    coefficient. (Mathematical formalism here would be quite ugly)}
  Then $(\dist{P}_{i, j, k})_{(i, j)} \approx \bigtimes_{(i, j)} \dist{U}_q$ if
  \begin{itemize}
  \item $\vec{B}$ has been picked uniformly at random,
  \item $\deg \p{b} (\vec{X}) \geq K$.
  \end{itemize}
\end{lemma}
