...
%  \item answer $\cdv$'s queries to $\oraclespcom$,
  ...
  Since $\ddv_{\pf}$ are algebraic, there exists extractors
  $\ext_{\ddv_{\pf}}(\srsPC, \qro_{\pf}, r_{\pf})$ that output
  $\p{f}$. Here $\qro_{\p{f}}$ is a set of all random oracle queries made by
  $\ddv_{\pf}$ and responses it got. As usual, $r_\pf$ denotes $\ddv_\pf$'s
  randomness.
