...
\begin{definition}[Proof system]
  ...
  \item[Prover]
    \begin{enumerate}
    \item[On input $\inp, \wit$] the prover picks randomly evaluation $\vec{b}$ of
      formal variable $\vec{B}$.
    \item[On protocol building challenge:] on partial transcript $T [i - 1]$ where
      the last verifier's message contains protocol building challenge $z_i$, the
      prover sends in Round $i$ messages $m_{i 1}, \ldots m_{i k_i}$ and sets up
      polynomial oracles for polynomials
      $\smallset{\p{p_{i}} (X, \vec{b})}_{i = 1}^{k'_i}$.
    \item[On evaluation challenge:]
    \end{enumerate}
  \item[Verifier]
    \begin{enumerate}
    \item[$i \leq \mu$:] on partial transcript $T [i - 1]$, prover's messages
      $m_{i 1}, \ldots m_{i k_i}$ and oracles
      $\smallset{\oracleo_{\p{p_{i}}}}_{i = 1}^{k'_i}$ sends in Round $i$ a
      \begin{itemize}
      \item protocol building challenge $\beta_i$, or
      \item evaluation challenge $\chz_i$.
      \end{itemize}
...
\begin{definition}[Zero knowledge]
  Let the protocol be a proof system as described in \cref{def:ps} and let
  $\smallset{\p{p_{i j}} (X, \vec{b})}$ be independent. Then  
\end{definition}
