...
Reduction $\bdv^{\oracles^{\PCOM}}$:
\begin{itemize}
\item On input $\inp$ from the adversary $\adv$ compute the simulated proof as
  follows, for each Round $i$:
  \begin{itemize}
  \item Ask the simulator for $\tilde{k}_i$ simulated commitments
    $c_{i, r_1}, \ldots, c_{i, r_{\tilde{k}}}$ that correspond to commitments to $\tilde{k}_i$
    randomized polynomials $\p{p}_{i, j}$ sent by the prover in the round. Pick $r_{i, j, k}$ according to
    $\dist{P}_{i', j}$ and save it as an evaluation of $\p{p}_{i, j}$ at $\vec{X_{i,
        j, k}}$ (note, the evaluation point may not be known yet).
  \item Compute the rest $k_i - \tilde{k}_i$ polynomial 
  \item Parse the protocol building and evaluation challenges $\vec{\beta_i}$,
    $\vec{x_{i', j, k}}$. 
  \item If the verifier asks to reveal evaluation of a randomized polynomial
    $\p{p_{i', j}} (\vec{x_{i', j, k}}, \cdot)$ pick $r_{i', j, k}$ according to
    $\dist{P}_{i', j}$ and output it.
  \item If the verifier asks to reveal evaluation of a determined polynomial
    ...
    the previously computed values (cf.~\cref{lem:rand_imp_det}) and output it.
