...
\begin{definition}[Whitebox Simulation extractable NIZK]
  ...
  Let $\oracles(\srs,td,\phi,w)$ be an oracle when $(\phi,w)\in \REL$ calls $\mathsf{SimProve(\srs,\tau,\phi)}$ internally, and also records $\phi$ in $Q$.
...
\item[Round 1] Sample $b_1, \ldots, b_9 \sample \FF_p$; compute
  $\p{a}(X), \p{b}(X), \p{c}(X)$ as
	...
	\end{multline*}
        Note that $\p{z}\in {\FF_{p}}_{\leq 3n}[X]$ satisfies the property that $\p{z}(\omega)=1$ and for $i>1$, $\p{z}(\omega^{i}) = \prod_{j=1}^{i} \p{f}(\omega^{i})/\p{g}(\omega^{i})$ where $\p{f} = \p{f_{a}}\p{f_{b}}\p{f_{c}}$ and $\p{g} = \p{g_{a}}\p{g_{b}}\p{g_{c}}$.
...
\begin{proof}
    ...
    \item Prepare a simulated proof for $\adv$ using $\oraclespcom$, that is
      ...
      \item Ask $\oraclespcom$ to evaluate all the polynomials at $\chz$ and get
        openings $W^{\p{f}}_\chz$, for
              ...
              \markulf{24.08}{We don't actually know the random polynomials, so we cannot compute their evaluations, but just simulating opening proofs for random $\ev{a},\dots$ should work.}
 ...
 \ncase{$\game{1} \mapsto \game{2}$} Probability that $\game{1}$ and $\game{2}$
  ...
  that
  \begin{itemize}
  \item internally run $\adv$ and $\bdv$,
  \item answer $\bdv$'s queries to $\oraclespcom$,
...
%  \item answers $\adv$, $\bdv$ queries to the random oracle by simulating it.
  ...
  The game returns $1$ if some of the extractors fail to output a correct
  ...
  runs $\cdv_{\p{f}}$ and $\ext_{\cdv_{\p{f}}}$, for $\p{f} \in \p{F}$. We show
  that if the adversary $\adv$ manages to produce a proof that verifies
  correctly, but the extracted polynomials do not encode a valid witness, then
  one can break PHP state-restoration soundness of the corresponding $\PHP$.
  \begin{enumerate}
  \item \label{it:a_b_c} Run $\cdv_{\p{a}}, \cdv_{\p{b}}, \cdv_{\p{c}}$ with
    $\ext_{\cdv_\p{a}}(\srsPC, {\qro}_{\p{a}}, r_{\p{a}}),
    \ext_{\cdv_\p{b}}(\srsPC, {\qro}_{\p{b}}, r_{\p{b}}),
    \ext_{\cdv_\p{c}}(\srsPC, {\qro}_{\p{c}}, r_{\p{c}})$ to learn polynomials
  ...
  \item On $\PHP.\verifier$ challenges $\beta, \gamma$,
    \begin{enumerate}
    \item Run $\cdv_{\p{z}}(\srsPC)$ such that on $\cdv_{\p{z}}$'s random oracle
    ...
    \item If $\cdv_{\p{z}}$ finalizes its proof for $\inp$ with $\cp{a}, \cp{b},
      \cp{c}$ then continue;
    \item Else, if $\cdv_{\p{z}}$ rewinds the proof and queries the random
      ...
      \item Go back to \cref{it:a_b_c} and run
        $\cdv_{\p{a'}}, \cdv_{\p{b'}}, \cdv_{\p{c'}}$ along with corresponding
        extractors to get $\p{a'}, \p{b'}, \p{c'}$.
      \end{enumerate}
    \end{enumerate}
  \item Run $\ext_{\p{z}}$ to learn $\p{z}$, send $\p{z}$ to $\PHP.\verifier$.
  \item On $\PHP.\verifier$ challenge $\alpha$.
     \begin{enumerate}
     \item Run
       $\cdv_{\p{t_1}}(\srsPC), \cdv_{\p{t_2}}(\srsPC), \cdv_{\p{t_3}}(\srsPC)$
       such that on $\cdv_{\p{t_i}}$'s, $\p{i} \in \range{1}{3}$, random oracle
       call on $(\inp, \cp{a}, \cp{b}, \cp{c}, \beta, \gamma, \cp{z})$ answer
       with $\alpha$;
     \item Append
       $(\inp, \cp{a}, \cp{b}, \cp{c}, \beta, \gamma, \cp{z}), (\alpha)$ to
       lists $\qro_{\p{f}}$, for $\p{f} \in \p{F}$.
     \item If all $\cdv_{\p{t_i}}$, $\p{i} \in \range{1}{3}$ finalizes its proof
       for $\inp$ with $\cp{a}, \cp{b}, \cp{c}, \beta, \gamma, \cp{z}$ then
       continue;
     \item Else, some $\cdv_{\p{t_i}}$, $\p{i} \in \range{1}{3}$ rewinds the
      ...
      \end{enumerate}
    \end{enumerate}
  \item Run $\ext_{\p{t_i}}$, $\p{i} \in \range{1}{3}$ to learn $\p{z}$, send
