...
\noindent\textbf{Building an extractor for SNARK from an extractor for PC}
  ...
  \ncase{Game 1} Let $\adv^{\oraclesps}(\srs, \srss, \radv)$ be a SE adversary
 ...
 % answers from the same distribution.
  ...
  \ncase{Game 2} Is the same as Game 1 except that we replace $\oraclesps$ with the real prover algorithm.
...
%  \michals{23.08}{Does $\cdv$ use the pcom simulator here?}\michals{25.08}{No.}
  ...
  \ncase{$\game{1} \mapsto \game{2}$} The difference in the winning probabilities between $\game{1}$ and $\game{2}$ is negligible by the zero-knowledge property of the prove system.

  %Probability that $\game{1}$ and $\game{2}$
  %return different values is $\epszk$.
