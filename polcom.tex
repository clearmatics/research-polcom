...
\begin{definition}[$\vec{Z}$-Zero knowledge]
  ...
  \item On verifier's evaluation query $\chz_{i, j, k}$, for $k \in \range{1}{K_{i, j}}$, to a
    ...
    \item Else, that is the queried oracle is for a polynomial in $D$, compute
      $p_{i, j, k} = \p{p}_{i, j} (\vec{x}, \vec{b})$ where $\vec{x}$ is w.r.t
      $\chz_k$ and \mxout{pick polynomial $\p{p}'_{i, j} (\vec{X}, \vec{B})$ such
        that $\p{p}'_{i, j} (\vec{x}, \vec{b}) = p_{i, j}$ and} respond with
      $p_{i, j, k}$. If the protocol does not contain any more evaluations of
      $\p{p}_{i, j} (\vec{x}, \vec{b})$, pick a polynomial $\p{p}'_{i, j} (\vec{x},
      \vec{b})$ that on $\chz_{i, j, k}$ evaluates to $p_{i, j, k}$, for $k \in
      \range{1}{K_{i, j}}$ and set an oracle for it.
    \end{itemize}
  \end{enumerate}
