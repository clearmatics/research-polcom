...
\item[$(\msg{commit})$:] Run $c \gets \simsample(\secparam)$, add $c$ to $Q_{com}$, return $c$.
  \item[$(\msg{open}, c, x', y)$:] If $c\in Q_{{com}}$ and $x'\neq x$, run
		$o\gets \simopen(\td,c,x,y)$, \michals{25.08}{$x'$ instead of $x$?}\antonio{02.09}{It's not clear
		what $x$ should be} add $(c,x',y,o)$ to $Q_{op}$, and return $o$. Otherwise return $\bot$.
  \end{description}
