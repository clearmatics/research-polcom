...
\begin{definition}[Flexible simulation extractability]
  ...
  Let $\PCOM = (\kgen, \com, \op, \verify)$ be a polynomial commitment scheme with a
  simulator $(\simgen, \simsample, \simopen)$. Let $\maxdeg$ be a maximal degree of
  polynomials that can be committed. $\oraclec$ take as input $c$. If
  $\Qchal[c] = \bot$, then $\oraclec$ samples $x$, sets $\Qchal[c] \gets x$ and returns $x$; if
  $\Qchal[c] = x \neq \bot$ it returns $x$. Let $\oracles$ be an oracle which on
  input
     \begin{description}
     \item[$(\msg{commit})$:] Run $c \gets \simsample (\secparam)$, add $c$ to
       $\Qcom$, return $c$.
...
\end{lemma}
\begin{proof}
  Here we build extractor $\ext_{\bdv_1}$ from $\ext_{\bdv_0}$.
\end{proof}
...
\begin{definition}[Flexible simulation extractability for vector of commitments]
  ...
  Let $\PCOM = (\kgen, \com, \op, \verify)$ be a polynomial commitment scheme with a
  simulator $(\simgen, \simsample, \simopen)$. Let $\maxdeg$ be a maximal degree of
  polynomials that can be committed. $\oraclec$ take as input $\vec{c}$. If
  $\Qchal[\vec{c}] = \bot$, then $\oraclec$ samples $x$, sets
  $\Qchal[\vec{c}] \gets x$ and returns $x$; if $\Qchal[\vec{c}] = x \neq \bot$ it
  returns $x$. Let $\oracles$ be an oracle which on input
     \begin{description}
     \item[$(\msg{commit})$:] Run $c \gets \simsample (\secparam)$, add $c$ to
       $\Qcom$, return $c$.
     \item[$(\msg{open}, c, x', y)$:] If $c \in \Qcom$ and $x' \neq x$, run
...
\end{lemma}
\begin{proof}
  Here we build extractor $\ext_{\bdv_2}$ from $\ext_{\bdv_1}$.
\end{proof}
...
\begin{definition}[Flexible simulation extractability for vector of commitments and
  ...
  simulator $(\simgen, \simsample, \simopen)$. Let $\maxdeg$ be a maximal degree of
  polynomials that can be committed.  $\oraclec$ take as input $\vec{c}$. If
  $\Qchal[\vec{c}] = \bot$, then $\oraclec$ samples $x$, sets $\Qchal[c] \gets x$ and
  returns $x$; if $\Qchal[c] = x \neq \bot$ it returns $x$.

  Let $\oraclec (\vec{c})$ be an oracle that takes
  a vector commitments $\vec{c}$, samples $x$, sets
  $\Qchal \gets \Qchal || (\vec{c}, x)$ and returns $x$. Let $\oracles$ be an oracle
  which on input
     \begin{description}
     \item[$(\msg{commit})$:] Run $c \gets \simsample (\secparam)$, add $c$ to
       $\Qcom$, return $c$.
     ...
     \[
       \Pr \left[
         ...
         \,\left|\, \vphantom{
           ...
           \begin{aligned}
             & (\srs, \td) \gets \simgen(\secparam, \maxdeg),\\
             & (\vec{c}, \vec{x}, \vec{y}, \vec{o}) \gets \bdv_3^{\oracles,\oraclec}(\srs; r), \\
             & x \in \Qchal[\vec{c}], \\
...
\end{lemma}
\begin{proof}
  Here we build extractor $\ext_{\bdv_3}$ from $\ext_{\bdv_2}$.
\end{proof}
...
\begin{definition}[Flexible simulation extractability with random oracle]
  ...
  We consider the same $\oracles$ and $Q_{sim}= (Q_{com},Q_{op})$ but an adversary that provides vectors of commitments and openings $\vec{c}, \vec{y}, \vec{o}$.
   Let $\ro_{aux}(\vec{c},aux)$ be an oracle that when $Q_{\ro}[\vec{c},aux] = \bot$, samples $x$, sets $Q_{\ro}[\vec{c},aux]=x$ and $q_{{chal}}= |Q_{{op}}|$. In all cases, it returns $Q_{\ro}[\vec{c},aux]$.
  We say that $\PCOM$ is \emph{vector $\ro_{aux}$ simulation extractable} if for any $\ppt$
  adversary $\bdv_4$ with oracle access to $\oracles$ and $\ro_{aux}$ there exists extractor
  $\ext_{\adv}$ such that
\[
  \Pr \left[
    \begin{aligned}
