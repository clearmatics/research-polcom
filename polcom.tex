...
\subsubsection{The idealised proof system}
  ...
  $\indexer$, prover $\prover$ and verifier $\verifier$, and simulator
  $\simulator$. At the beginning, the indexer $\indexer$ provides index $\ind$,
  prover $\prover$ sends in each Round $i$ messages $m_{i, 1}, \ldots m_{i k'_i}$ and
  ...
  The verifier sends challenges, which we divide into two groups: \emph{protocol
    building challenges} $\beta_{i, j}$ where index $(i, j)$ should be interpreted as
  $j$-th challenge in Round $i$ and \emph{evaluation challenges} $\vec{x_{i, j, k}}$,
  where index $(i, j, k)$ means $k$-th challenge to polynomial $\p{p}_{i, j}$. Vector
  $\vec{X}$ denotes a vector of random variables corresponding to evaluation
  challenges.
  % That is, for each evaluation challenge $\vec{x_{i, j, k}}$ the protocol
  % specifies which $X \in \vec{X}$ are realised by $\chz_{i, j, k}$. 
  ...
  end of the list of polynomials $\p{p}_{i, j} (\vec{X}, \vec{B})$. From now one, we
  ...
  \item Let $\p{p}_{i', j'} (\vec{X}, \vec{B})$ be a polynomial that is evaluated at
    ...
    $B_{i_k}$ is zero. We denote the set of randomized polynomials by $R$.

    We say that $\p{p}_{i', j'}$ \emph{introduces} $B_i$ if for all
    $(i, j) < (i', j')$ coefficient of $B_i$ in $\p{p}_{i, j}$ is zero. Hence, a
    randomised polynomial evaluated at $k$ points is a polynomial that introduces at
    least $k$ randomizers $B_i$

    Importantly, we assume that there are no polynomials $\p{p}_{i', j'}$ which are
    evaluated at $k$ points but there is only $k'$, for $k' \in \range{1}{k - 1}$
    elements $B_{i_1}, \ldots, B_{i_k'} \in \smallset{B_1, \ldots, B_l}$ such that
    $\p{p}_{i', j'} (\vec{X}, \vec{B})$ has non-zero coefficient next to $B_{i_k}$
    while for all $\p{p}_{i, j}$ for $(i, j) < (i', j')$, coefficient next to
    $B_{i_k}$ is zero. That is, we assume that there is no polynomials that are
    evaluated at $k$ points but which introduce non-zero, but less than $k$
    randomisers.
  ...
  \end{itemize}
    ...
    % $\p{p}_{i', j'} (\vec{X}, \vec{B})$ has non-zero coefficient next to $B_i$ while
    % for all $\p{p}_{i, j}$ for $(i, j) < (i', j')$, coefficient next to $B_i$ is
    % zero.  Polynomials $\p{p}_{i, j} (\vec{X}, \vec{B})$ that are not
    % \emph{randomised polynomials} are called \emph{determined}.  \fi

    Importantly, the following lemma holds.

    \begin{lemma}
      \label{lem:matching_R_polys}
      Let $\p{p}_{i', j'} (\vec{X}, \vec{B})$ be a polynomial that is to be evaluated
      at $k$ points $\vec{x_1}, \ldots, \vec{x_k}$. For selection of $\vec{x_i}$,
      randomness $\vec{b}$ and randomised polynomial $\p{p}(\vec{X}, \vec{B})$ there
      is a PPT algorithm $\adv$ that outputs a rational function
      $\p{p}'(\vec{X}, \vec{B})$ such that
      $\p{p}(\vec{x}, \vec{b}) = \p{p}(\vec{x}, \vec{b})$.
    \end{lemma}
    \begin{proof}
      Let $\p{p} (\vec{x_i}, \vec{b}) = r_i$. Let $\adv$ be an algorithm that picks
      coefficients of $\p{p}'' (\vec{X}, \vec{B})$ randomly and defines
      $\p{p}' (\vec{X}, \vec{B}) = \p{p}'' (\vec{X}, \vec{B}) \cdot (X_1 \cdot \ldots
      \cdot X_{l'} - x_{j1} \cdot \ldots \cdot x_{jl'}) + b_i (\vec{X})$ where
      $b_i (X)$ is defined as follows:
    \[
      b_i (\vec{X}) = r_i \prod_{j = 1, j \neq i}^{k} \frac{(X_1 \cdot \ldots \cdot
        X_{l'} - x_{j1} \cdot \ldots \cdot x_{jl'})}{(x_{i1} \cdot \ldots \cdot
        x_{il'} - x_{j1} \cdot \ldots \cdot x_{jl'})}
    \]
    That is for $b_i (\vec{x_i}) = r_i$ and $b_i (\vec{x_j}) = 0$, for $j \neq
    i$. Eventually $\p{p}' (\vec{x_i}, \vec{b}) = r_i$.
  \end{proof}

  Note that polynomial $\p{p}_{i', j'} (\vec{X}, \vec{B})$ can be written as
  \[
    \p{p}_{i', j'} (\vec{X}, \vec{B}) = \p{h}_{i', j'}\left(\vec{X}, \vec{\beta},
      (\p{p}_{i, j} (\vec{X}, \vec{B}))_{(i, j) \leq (i', j')} \right)
  \]
  \michals{9.12}{Note that in the definition above i removed $\vec{B}$. That is,
    polynomial $h$ depends on $\vec{B}$ only indirectly, i.e.~via polynomials
    $p$. Formalize that}

  Let
  $\p{p}'_{i, j} (\vec{X}, \vec{B}) = \p{h}_{i, j}\left(\vec{X}, \vec{\beta},
    (\p{p}'_{i, j} (\vec{X}, \vec{B}))_{(i, j) \leq (i', j')} \right) $ be
  polynomials such that $\p{p}'_{i, j} (\vec{x}, \vec{b'}) = \p{p}_{i, j} (\vec{x}, \vec{b})$
  for $\p{p}_{i, j} \in R$. Observe that for all $\p{p}_{i, j} \in D$ hold
  \[
    \p{p}_{i, j} (\vec{x}, \vec{b}) = \p{h}_{i, j}\left(\vec{x}, (\p{p}'_{i, j}
      (\vec{x}, \vec{b'}))_{(i, j) \leq (i', j')} \right).
  \]

  That is, if $\adv$ produces polynomials $\p{p}'_{i, j}$, $(i, j) \in R$, as in
  \cref{lem:matching_R_polys} then $\p{p}_{i, j} (\vec{x}, \vec{b}) = \p{p}'_{i, j}
  (\vec{x}, \vec{b'})$.


  We now present more detailed description of the actors.
  % \begin{description}
  ...
  \paragraph{Prover.}
    ...
    \item Denote by $K_{i, j}$ the number of evaluation queries the verifier makes to
      polynomial $\p{p}_{i, j}$. If for polynomial $\p{p}_{i, j}$ the verifier sent
      all $K_{i, j}$ evaluation challenges $\vec{x_{i, j, k}}$, the prover computes
      $p_{i, j, k} = \p{p}_{i, j} (\vec{x_{i, j, k}}, \vec{b})$ and sends it to the
      verifier. \michals{10.12}{Requiring that we send evaluations only after all
        evaluation points are known is needed by zero knowledge}
    \end{enumerate}
  ...
  \item[$i \leq \mu$:] on partial transcript $\zkproof [i - 1]$, prover's messages
    ...
    $i$:
    \begin{itemize}
    \item a protocol building challenge $\smallset{\beta_{i, j}}_j$,
    \item a evaluation challenge $\smallset{\vec{x_{i', j, k}}}_{k = 1}^{K_{i', j}}$.
    \end{itemize}
  ...
  \paragraph{Simulator.}
\michals{10.12}{We need to require that the polynomial are evaluated after all
  evaluations queries for that polynomial are known! (Otherwise we can easily have a
  HVZK -- the simulator would just pick the evaluation challenges randomly), but
  showing ZK may be difficult.}
  ...
  denote by and $R$ (resp.~$D$) the set of all randomized (resp.~determined)
  polynomials in $\PS$.

  The simulator $\simulator$ for instance $\inp$ picks a random witness $\wit$,
  and in Round $i$ proceeds as follows:
  \begin{enumerate}
  \item \michals{10.12}{Question -- the simulator picks polynomials after evaluations
      $\vec{x}$ are known. What if messages $\p{p}_{i, j}$,
      $j = k_i + 1, \ldots, k_i + k'_i$ depends on those polys?} For
    $j = k_i + 1, \ldots, k_i + k'_i$, send to the verifier polynomials
  ...
  \item On verifier's evaluation query $\vec{x_{i, j, K_{i, j}}}$, i.e. for the last
    ...
    \begin{itemize}
    \item Pick rational functions $\smallset{\p{p}_{i, j}}_{(i, j) \in R}$ as
      described in \cref{lem:matching_R_polys}.
    \item Compute rational functions $\smallset{\p{p}_{i, j}}_{(i, j) \in D}$
      according to the protocol.
    \item Send evaluations $\p{p}' (\vec{x}, \vec{b'})$, for
      $\vec{x} \in \smallset{\vec{x_{i, j, k}}}_{k = 1}^{K_{i, j}}$.
    \end{itemize}
  \end{enumerate}
...
\begin{theorem}[$\vec{Z}$-Zero knowledge]
  ...
  %   $\wit$. \michals{1.12}{We assume that the instance $\inp$ is part of the transcript.}
  ...
  %   \end{itemize}
  % \end{enumerate}
  \michals{29.11}{Here I say nothing about evaluation equation to be
    fulfilled. However, this is obvious as we pick evaluations of polynomials in $D$
    in a way an honest prover would pick, hence the ver equation holds directly from
