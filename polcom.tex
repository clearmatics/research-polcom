...
prover $\prover (\ind, \inp, \wit)$ sets an oracle for. Denote by $\dist{P}_{i, j, k}$ a distribution such that
\[
  \left\{ (\p{p}_{i, j} (\vec{x_{i, j, k}}, \vec{b}))_{(i, j)}\right\}_{\vec{x_{i, j,
        k}}} \approx \dist{P}_{i, j, k}.
\]
   ...
   The following lemma holds.
    ...
    \begin{lemma}
      ...
      Let $r_{i, j, k} \in \FF$ be an evaluation of a randomised polynomial
      $\p{p}_{i, j} (\vec{X}, \vec{B})$ at some evaluation point $\vec{x_{i, j, k}}$. Then
      there is a a PPT algorithm $\adv$ that outputs a rational function
      $\p{p}'_{i, j} (\vec{X}, \vec{B})$ and vector $\vec{b'}$ such that
      $\p{p}_{i, j}(\vec{x}, \vec{b}) = \p{p}'(\vec{x}, \vec{b'})$.
    \end{lemma}
    \begin{proof}
    ...
    follows:
    \begin{align*}
      \p{r} (\vec{X}, \vec{B}) & = \p{p}''_{i, j} (\vec{X}, \vec{B}) \cdot \prod_{k = 1}^{K_{i, j}} (X_1 \cdot \ldots \cdot X_{l'} - x_{i, j, k, 1} \cdot \ldots \cdot x_{i, j, k, l'}) \\
      b_{i, j, k} (\vec{X}) & =  r_{i, j, k} \prod_{k' = 1, k' \neq k}^{K_{i, j}} \frac{(X_1 \cdot \ldots \cdot
                              X_{l'} - x_{i, j, k', 1} \cdot \ldots \cdot x_{i, j, k', l'})}{(x_{i, j, k' 1} \cdot \ldots \cdot
                              x_{i, j, k', l'} - x_{i, j, k', 1} \cdot \ldots \cdot x_{i, j, k', l'})}
    \end{align*}
      
    That is, for all $\vec{x_{i, j, k}}$ polynomial $\p{r} (\vec{X}, \vec{B})$
zeroes; and $b_{i, j, k} (\vec{x_{i, j, k}}) = r_{i, j, k}$ and
    $b_{i, j, k} (\vec{x_{i, j, k'}}) = 0$, for $k' \neq k$. Eventually
    $\p{p}'_{i, j, k} (\vec{x_{i, j, k}}, \vec{b}) = r_{i, j, k}$. \qed
  \end{proof}

  \begin{lemma}
    Let $r_{i, j, k} = \p{p}_{i', j'} (\vec{x_{i, j, k}}, \vec{b})$ and let
    $\p{p}'_{i', j'} (\vec{X}, \vec{B})$ be such that
    $\p{p}'_{i', j'} (\vec{x_{i, j, k}}, \vec{b'}) = r_{i, j, k}$ for all
    $(i', j') \in R$. Then for all $(i', j') \in D$ holds
    \[
      \p{p}_{i', j'} (\vec{x_{i, j, k}}, \vec{b}) =\p{p}'_{i', j'} (\vec{x_{i, j,
          k}}, \vec{b'}).
    \]
    That is, if $\adv$ produces polynomials $\p{p}'_{i, j}$, for $(i, j) \in R$, as
    in \cref{lem:matching_R_polys} then
    $\p{p}_{i, j} (\vec{x}, \vec{b}) = \p{p}'_{i, j} (\vec{x}, \vec{b'})$.
  \end{lemma}
  \begin{proof}
  ...
  \[
    \p{p}_{i', j'} (\vec{X}, \vec{B}) = \p{h}_{i', j'}\left(\vec{X},
      (\vec{\beta_i})_{i \leq i' - 1}, (\p{p}_{i, j} (\vec{X}, \vec{B}))_{(i, j) \leq
        (i', j')} \right).
  \]
  Also note that there is a minimal index $(i, j) \in D$ such that for all $(i^*, j^*)
  \leq (i, j)$ polynomials $\p{p}_{i^*, j^*}$ are randomised.
 % 
  Hence by an induction argument over $(i', j')$ for all $(i', j') \in D$ holds
  \begin{equation*}
  \begin{split}
    \p{p}_{i', j'} (\vec{x_{i, j, k}}, \vec{b}) = \p{h}_{i', j'}\left(\vec{x_{i, j, k}},
      (\vec{\beta_i})_{i \leq i' - 1}, (\p{p}_{i, j} (\vec{x_{i, j, k}}, \vec{b}))_{(i, j) \leq
        (i', j')} \right) =
    \\ \p{h}_{i', j'}\left(\vec{x_{i, j, k}},
      (\vec{\beta_i})_{i \leq i' - 1}, (\p{p}'_{i, j} (\vec{x_{i, j, k}}, \vec{b'}))_{(i, j) \leq
        (i', j')} \right) = \p{p}'_{i', j'} (\vec{x_{i, j, k}}, \vec{b'})
  \end{split}
\end{equation*}
\end{proof}
  ...
  \begin{enumerate}
  \item
    % \michals{10.12}{Question -- the simulator picks polynomials after evaluations
    %   $\vec{x}$ are known. What if messages $\p{p}_{i, j}$,
    %   $j = k_i + 1, \ldots, k_i + k'_i$ depends on those polys?}
    \michals{13.12}{Since the simulator picks polynomials in $R$ randomly, we probably
      doesn't need to wait till it learns all evaluation points to reveal evaluations}
    For $j = k_i + 1, \ldots, k_i + k'_i$, send to the verifier polynomials
  ...
  \item On verifier's evaluation query $\vec{x_{i, j, K_{i, j}}}$, i.e. for the last
    ...
    \item Pick evaluations $r_{i, j, k}$ randomly w.r.t distribution
      $\dist{P}_{i, j, k}$.
    \item Compute polynomials $\smallset{\p{p}'_{i, j}}_{(i, j) \in R}$ as described
...
\begin{proof}
  ...
  random witness $\wit'$ and evaluations $r_{i, j, k}$ there exists a real prover
  that for instance $\inp$, witness $\wit$ and randomness $\vec{b}$ sets oracles for
  polynomials $\p{p}_{i, j}$ such that
